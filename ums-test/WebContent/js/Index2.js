...
function openadd() {
    ...
    $("input").val("");
    $.post("/ums-test/user/list",{},function(data){
    	var users = data.users;
    	for(var i=0;i<users.length;i++){
    		$("#user").append("<option value='"+users[i].uid+"'>"+users[i].username+"</option>");
    	}
    	/*data.each(function(i,n){
    		$("#user").append("<option value='"+n.users.uid+"'>"+n.users.username+"</option>");
    	});*/
...
function add() {
    ...
    $.ajax({
        url: "/ums-test/customer/addCustomer",
        timeout: 300000,
        /*dataType: "json",*/
        type: "post",
